...
def get_pt_bus(car_fingerprint, has_relay=False):
...
def create_acc_commands(packer, enabled, accel, gas, idx, stopping, starting, car_fingerprint, has_relay=False):
  commands = []
  bus = get_pt_bus(car_fingerprint, has_relay)
...
def create_bosch_supplemental_1(packer, car_fingerprint, idx, has_relay=False):
  # non-active params
  values = {
    "SET_ME_X04": 0x04,
    "SET_ME_X80": 0x80,
...
def spam_buttons_command(packer, button_val, idx, car_fingerprint, has_relay=False):
  values = {
